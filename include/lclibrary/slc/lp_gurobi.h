 ...
 * The file contains SLC LP formulation solved using Gurobi
 *
 * TODO:
 *
...
#ifndef LCLIBRARY_SLC_LP_GUROBI_H_
#define LCLIBRARY_SLC_LP_GUROBI_H_
...
namespace lclibrary {
	...
	class SLC_LP_Gurobi : public SLC_LP {
		size_t n_, m_, m_nr_;
		size_t depot_index_;
		double obj_val_;
		...
		public:
		SLC_LP_Gurobi(const std::shared_ptr <const Graph> g_in) : g_{g_in} {
		...
		void AddVars() {
			...
			for(size_t i = 0; i < m_; ++i) {
				req_sa_[i] = model->addVar(0.0, 1.0, g_->GetServiceCost(i), GRB_CONTINUOUS, "s_" + std::to_string(i));
				req_sa_rev_[i] = model->addVar(0.0, 1.0, g_->GetReverseServiceCost(i), GRB_CONTINUOUS, "s_" + std::to_string(i) + "_rev");
		...
		int GenerateSolutionGraph (std::shared_ptr <Graph> &digraph, std::shared_ptr <Graph> &undirected_graph) {
			...
			for(size_t i = 0; i < m_; ++i) {
				...
				else if(IsNearZero(s_a_rev - 1.0, 1e-1)) {
					Edge new_edge(edge);
...
#endif /* LCLIBRARY_SLC_LP_GUROBI_H_ */
